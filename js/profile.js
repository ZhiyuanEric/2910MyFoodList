function editing() {
    ...
    $('.likesDiv').each(function() {
        if ($('.likesDiv .btn').length < $('.likesDiv').length) {
            $(this).append('<button type="button" id="likesDelete" class="btn btn-danger col-xs-1">-</button>')
        }
    });
    $('.dislikesDiv').each(function() {
        if ($('.dislikesDiv .btn').length < $('.dislikesDiv').length) {
            $(this).append('<button type="button" id="dislikesDelete" class="btn btn-danger col-xs-1">-</button>')
        }
    });
    $('.allergiesDiv').each(function() {
        if ($('.allergiesDiv .btn').length < $('.allergiesDiv').length) {
            $(this).append('<button type="button" id="allergiesDelete" class="btn btn-danger col-xs-1">-</button>')
        }
    });
}
    ...
    $('#moreLikes, #moreDislikes, #moreAllergies').on('click', function() {
        if (this.id == 'moreLikes') {
            if ($('#likes').val() == '') {
                ...
                $('#likesError').html('');
                $('<div class="row likesDiv"><li class="list-group-item col-xs-11 newLikes">' + $('#likes').val() +
                '</li><button type="button" id="likesDelete" class="btn btn-danger col-xs-1">-</button></div>').insertBefore('#likesHidden');
                $('#likes').val('');
            }
            ...
            if ($('#dislikes').val() == '') {
                ...
                $('#dislikesError').html('');
                $('<div class="row dislikesDiv"><li class="list-group-item col-xs-11 newDislikes">' + $('#dislikes').val() +
                '</li><button type="button" id="dislikesDelete" class="btn btn-danger col-xs-1">-</button></div>').insertBefore('#dislikesHidden');
                $('#dislikes').val('');
            }
            ...
            if ($('#allergies').val() == '') {
                ...
                $('#allergiesError').html('');
                $('<div class="row allergiesDiv"><li class="list-group-item col-xs-11 newAllergies">' + $('#allergies').val() +
                '</li><button type="button" id="allergiesDelete" class="btn btn-danger col-xs-1">-</button></div>').insertBefore('#allergiesHidden');
                $('#allergies').val('');
            }
        }
    });
